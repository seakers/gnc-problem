function [out1, out2] = GNC_plot_minNSNC(GNC)
...
%% Second plot, with colors according to min NS NC
[m1, N91, ind1] = GNC_archs_with_minNSNC(GNC,1);
[m2, N92, ind2] = GNC_archs_with_minNSNC(GNC,2);
[m3, N93, ind3] = GNC_archs_with_minNSNC(GNC,3);

figure;
plot(N91, m1, 'b+', 'LineWidth', 2);
hold on;
plot(N92, m2, 'r+', 'LineWidth', 2);
plot(N93, m3, 'g+', 'LineWidth', 2);

xlabel(txt1,'Fontsize',12);
ylabel(txt2,'Fontsize',12);
title(fig_name,'Fontsize',15);
legend({'min(NS,NC)=1','min(NS,NC)=2','min(NS,NC)=3'});

%% Third plot, with markers according to min NS NC
figure;
plot(N91, m1, 'ko', 'LineWidth', 1);
hold on;
plot(N92, m2, 'kx', 'LineWidth', 1);
plot(N93, m3, 'k.', 'LineWidth', 1);
